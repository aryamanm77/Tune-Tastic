...
import { Home, Search, Library, PlusSquare, Heart, Sliders, Sparkles, Mic } from 'lucide-react';
import { usePlayer } from '../context/PlayerContext';
import TuneTasticLogo from './TuneTasticLogo';

interface SidebarProps {
  activeTab: string;
  setActiveTab: (tab: 'home' | 'search' | 'library' | 'playlist' | 'dj' | 'premium' | 'studio') => void;
    ...
    <div className="sidebar hide-mobile">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '16px' }}>
        ...
        <button 
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '14px', fontWeight: 'bold', color: activeTab === 'studio' ? 'var(--text-primary)' : 'var(--text-secondary)' }}
        >
          <Mic size={24} /> Music Studio
        </button>
      </div>
